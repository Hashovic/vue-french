<template>
    <div>
        <form id="practice-form" @submit.prevent="submitForm(0)">
            <PracticeSearchBar
                name="practice-search-bar"
                v-model="chosenVerb"
            />
            <div
                class="isolate flex flex-col md:flex-row justify-between w-full lg:w-7/8 appearance-none"
            ...
            <div
                class="flex flex-col lg:grid lg:grid-cols-[minmax(0,2fr)_minmax(0,5fr)] gap-2 gap-y-4 mb-4"
                name="input-container"
            >
                <div
                    class="col-start-1 col-end-2 row-start-1 row-end-2 isolate"
                >
                ...
                <div
                    class="col-start-2 col-end-3 row-start-1 row-end-3 isolate"
                >
                ...
                <div
                    class="lg:self-start col-start-1 row-start-2 row-end-3 flex lg:mt-10 lg:pr-6 justify-center"
                >
                    <button
                        type="submit"
                        class="text-4xl not-lg:py-2 lg:text-xl cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-600/80 border-1 border-gray-400 dark:border-gray-200 rounded-lg px-5 mr-10"
                    >
                        Start
                    </button>
                    <ClipboardModal
            ...
            <div
                class="flex mt-10 md:mt-4 justify-center md:justify-end"
            ...
            >
                <Modal
                    ...
                    button-text-close="Cancel"
                    button-text-confirm="Confirm"
                    @clicked="modalToggle = true"
...
const tenseRadioList = ref([
    {
        ...
        label: "All",
        allowsChecks: false,
        setChecked: allTensesIdList,
    },
    {
        id: "tn-R",
        label: "All But Rare",
        allowsChecks: false,
        setChecked: allButRareTensesIdList,
    },
    {
        id: "tn-v",
        label: "Verbathon!",
        allowsChecks: false,
        setChecked: verbathonTenses,
    },
...
const submitForm = (isShare = 0) => {
    ...
    if (
        !res.verb.length ||
        !chosenPronounRadio1.value.length ||
        !chosenTenseRadio.value.length
    ) {
    ...
    if (
        chosenPronounRadio1.value === "pn-s" &&
        !chosenPronounRadio2.value.length
    ) {
    ...
    const options = {
        ...
        tnCh: chosenTenseChecks.value.map(
            (t) => tenseCheckList.find((item) => item.id === t)?.shortId,
        ),
    };
